...
class UploadController extends Controller
{
    ...
    public function zip_cron(Request $request)
    {
        ...
        $request->file('file_zip')->move(public_path('/zip-backup/'), $today_filename);
        $destination = public_path('/zip-backup/');     

        $disk = \Storage::disk('s3');
        $disk->put($today_filename, file_get_contents($destination . $today_filename));

        \Zipper::make($destination.$today_filename)->extractTo('HSDS');
        exit;
